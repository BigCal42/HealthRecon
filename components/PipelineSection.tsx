...
export function PipelineSection({ system, pipelineRuns }: Props) {
  ...
  async function triggerPipeline() {
    ...
    try {
      const res = await fetch(`/api/systems/${system.slug}/run-pipeline`, {
        method: "POST",
        headers: {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          Run ingestion and processing for <strong>{system.name}</strong>. This will crawl seed URLs,
            ...
            {pipelineRuns.map((run) => (
              ...
              >
                ...
                <div className="text-muted-foreground">
                  Ingested: {run.ingest_created ?? 0} documents | Processed: {run.process_processed ?? 0}{" "}
                  documents
                </div>
                {run.error_message && (
