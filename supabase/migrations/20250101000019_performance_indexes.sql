...
-- Migration: 20250101000019_performance_indexes
...
-- System narratives table indexes
...
-- Opportunity suggestions table indexes
...
-- Outbound playbooks table indexes
...
create index if not exists pipeline_runs_system_id_idx on pipeline_runs (system_id);
...
-- Daily briefing runs table indexes
create index if not exists daily_briefing_runs_system_id_idx on daily_briefing_runs (system_id);
create index if not exists daily_briefing_runs_status_idx on daily_briefing_runs (status);
create index if not exists daily_briefing_runs_created_at_idx on daily_briefing_runs (created_at);
